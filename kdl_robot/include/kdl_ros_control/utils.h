                                                                 ...
                                                                 const Eigen::Matrix<double,3,3> &_R_e)
{
   ...
   // return _omega_d - _omega_e;
   /*Eigen::Quaterniond q_e(_R_e);
   Eigen::Quaterniond q_d(_R_d);
   Eigen::Matrix<double,3,1> eps_e;
   eps_e << q_e.x(), q_e.y(), q_e.z();
   Eigen::Matrix<double,3,1> eps_d;
   eps_d << q_d.x(), q_d.y(), q_d.z();
   double eta_e = q_e.w();
   double eta_d = q_d.w();
   Eigen::Matrix3d I = Eigen::Matrix3d::Identity();
   
   return -0.5*eps_e.transpose()*_omega_e*eps_d + 0.5*eta_e*(eta_d*I - skew(eps_d))*_omega_d + 0.5*eps_d.transpose()*_omega_d*eps_e - 0.5*eta_d*(eta_e*I - skew(eps_e))*_omega_e - skew(0.5*(eta_d*I - skew(eps_d))*_omega_d)*eps_e - 0.5*skew(eps_d)*(eta_e*I - skew(eps_e))*_omega_e;*/
}
...
inline Eigen::MatrixXd gradientJointLimits(const Eigen::VectorXd &q, const Eigen::MatrixXd &jntLimits, double &costValue)
{
    ...
    for(unsigned int i = 0; i < n; i++)
    {
        ...
        double q_mid = (jntLimits(i,1) - jntLimits(i,0)) / 2;
        gradient(i) = - (1/n) * (q(i) - q_mid) / std::pow(jntLimits(i,1) - jntLimits(i,0),2);
    }
    return gradient;
}
